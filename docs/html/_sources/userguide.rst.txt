...
`hesseflux_example.cfg` in the `example` directory is highly commented and should be (almost)
...
`hesseflux_example.cfg` looks like:
...
`postproc_europe-fluxdata.py` checks the units of air temperature (i.e. the first column
starting with `TA_`).
...
two-week chunks. The section in `hesseflux_example.cfg` looks like:
...
for other land cover types. `postproc_europe-fluxdata.py` sets 0.01 as its default value.
...
`postproc_europe-fluxdata.py` also takes as its default.
...
The script `postproc_europe-fluxdata.py` uses the function :func:`gapfill` to calculate flux
uncertainties like:
