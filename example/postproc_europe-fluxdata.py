...
The script is controlled by a config file in Python's standard configparser format.
The config file includes all possible parameters of used routines. Default
parameter values follow the package REddyProc where appropriate. See comments
in config file for details.

The script currently flags on input all NaN values and given *undefined* values.
Variables should be set to *undefined* in case of other existing flags before
calling the script. Otherwise it should be easy to set the appropriate flags in
the pandas DataFrame dff for the flags after its creation around line 160.
...
def _findfirststart(starts, names):
    ...
    >>> hout = _findfirststart(['TA', 'LE'],
                               ['TIMESTAMP', 'TAU_1_1_1', 'H_1_1_1', 'LE_1_1_1',
...
if __name__ == '__main__':
    ...
    if ',' in eufluxfile:
        ...
        if eufluxfile:
            ...
            try:
                eufluxfile = hf.files_from_gui(initialdir='.', title='europe-fluxdata.eu file(s)')
            except:
    ...
    if len(eufluxfile) > 1:
        for infile in eufluxfile[1:]:
            df1 = pd.read_csv(infile, sep, skiprows=skiprows, parse_dates=[0], date_parser=parser, index_col=0, header=0)
    ...
    t02 = ptime.time()
    strin = '[m]: {:.1f}'.format((t02-t01)/60.) if (t02-t01)>60. else '[s]: {:d}'.format(int(t02-t01))
    print('   in ', strin)
    ...
    if outlier:
        ...
        t12 = ptime.time()
        strin = '[m]: {:.1f}'.format((t12-t11)/60.) if (t12-t11)>60. else '[s]: {:d}'.format(int(t12-t11))
        print('   in ', strin)
    ...
    if ustar:
        ...
        for ii, hh in enumerate(hout):
            dff.loc[flag==2, hh] = 5
        t22 = ptime.time()
        strin = '[m]: {:.1f}'.format((t22-t21)/60.) if (t22-t21)>60. else '[s]: {:d}'.format(int(t22-t21))
        print('   in ', strin)
    ...
    if partition:
        ...
        t42 = ptime.time()
        strin = '[m]: {:.1f}'.format((t42-t41)/60.) if (t42-t41)>60. else '[s]: {:d}'.format(int(t42-t41))
        print('   in ', strin)
    ...
    if fill:
        ...
        t32 = ptime.time()
        strin = '[m]: {:.1f}'.format((t32-t31)/60.) if (t32-t31)>60. else '[s]: {:d}'.format(int(t32-t31))
        print('   in ', strin)
    ...
    if fluxerr:
        ...
        t52 = ptime.time()
        strin = '[m]: {:.1f}'.format((t52-t51)/60.) if (t52-t51)>60. else '[s]: {:d}'.format(int(t52-t51))
        print('   in ', strin)
    ...
    if not outputfile:
        try:
            outputdir = hf.directory_from_gui(initialdir='.', title='Output directory')
        except:
            raise IOError("GUI for output directory failed.")
        outputfile = outdir + '/' + os.path.basename(configfile[:configfile.rfind('.')]+'.csv')
    ...
    t62 = ptime.time()
    strin = '[m]: {:.1f}'.format((t62-t61)/60.) if (t62-t61)>60. else '[s]: {:d}'.format(int(t62-t61))
    print('   in ', strin)
